...
export const useDataStore = defineStore("dataStore", () => {
  ...
  async function fetchAllLandSurveyData(
    filters: Record<string, any> = {},
    pageSize = 500
  ) {
    let page = 1;
    ...
    while (page <= maxPage) {
      const res = await lotSurveyApi.getLotSurveyData({
        ...filters,
        page,
        pageSize,
      });
  ...
  async function exportLandSurveyExcel(filters: Record<string, any> = {}) {
    const allData = await fetchAllLandSurveyData(filters);

    const ws = XLSX.utils.json_to_sheet(allData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Survey Data");

    XLSX.writeFile(wb, "Land_Survey.xlsx");
  }
  return {
    fetchAllBuildingSurveyData,
    exportBuildingSurveyExcel,

    fetchAllLandSurveyData,
    exportLandSurveyExcel,
  };
});
